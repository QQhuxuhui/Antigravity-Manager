...
/// Holds one in-flight scheduling slot for an account.
///
/// The guard is intentionally RAII-based so all retry/early-return paths release
/// the slot without each handler needing manual cleanup.
#[derive(Debug)]
pub struct AccountLoadGuard {
    account_id: String,
    counter: Arc<AtomicUsize>,
}

impl Drop for AccountLoadGuard {
    fn drop(&mut self) {
        let previous = self
            .counter
        ...
        if matches!(previous, Ok(0)) {
            tracing::warn!(
                "Account load guard for {} observed an already-empty counter",
                self.account_id
            );
        }
    }
...
pub struct TokenManager {
    ...
    account_inflight: Arc<DashMap<String, Arc<AtomicUsize>>>, // account_id -> current in-flight requests
    account_request_windows:
        Arc<DashMap<String, Arc<std::sync::Mutex<VecDeque<std::time::Instant>>>>>, // local RPM precheck
    model_capacity_cooldowns: Arc<DashMap<String, std::time::Instant>>, // model -> cooldown until
...
impl TokenManager {
    ...
    const MODEL_CAPACITY_COOLDOWN_SECS: u64 = 10;
    ...
    pub fn new(data_dir: PathBuf) -> Self {
        Self {
            ...
            account_request_windows: Arc::new(DashMap::new()),
            model_capacity_cooldowns: Arc::new(DashMap::new()),
    ...
    fn account_inflight_count(&self, account_id: &str) -> usize {
    ...
    fn is_flash_model(model: &str) -> bool {
        ...
        lower.contains("flash") || lower.contains("haiku")
    }

    fn local_rpm_limit(token: &ProxyToken, model: &str) -> Option<usize> {
        let tier = token
            .subscription_tier
            .as_deref()
            .unwrap_or("")
            .to_ascii_uppercase();
        let is_flash = Self::is_flash_model(model);

        if tier.contains("ULTRA") {
            Some(120)
        } else if tier.contains("PRO") {
            Some(120)
        } else if tier.contains("FREE") || tier.contains("LEGACY") {
            Some(if is_flash { 15 } else { 2 })
        } else {
            Some(if is_flash { 15 } else { 2 })
        }
    }

    fn local_rpd_limit(token: &ProxyToken, model: &str) -> Option<u64> {
        let tier = token
            .subscription_tier
            .as_deref()
            .unwrap_or("")
        ...
        if tier.contains("ULTRA") {
            ...
            Some(if is_flash { 4000 } else { 1500 })
        } else if tier.contains("FREE") || tier.contains("LEGACY") {
            Some(if is_flash { 1500 } else { 50 })
        } else {
            Some(if is_flash { 1500 } else { 50 })
        }
    }

    fn current_local_rpm(&self, account_id: &str) -> usize {
        let Some(window) = self.account_request_windows.get(account_id) else {
            return 0;
        };
        let cutoff = std::time::Instant::now()
            .checked_sub(std::time::Duration::from_secs(Self::LOCAL_RPM_WINDOW_SECS))
    ...
    fn record_local_dispatch(&self, account_id: &str) {
        let window = self
            .account_request_windows
    ...
    fn model_capacity_remaining_wait(&self, model: &str) -> u64 {
        let Some(until) = self.model_capacity_cooldowns.get(model).map(|v| *v) else {
            return 0;
        };
        let now = std::time::Instant::now();
        if until <= now {
            self.model_capacity_cooldowns.remove(model);
            return 0;
        }
        until.duration_since(now).as_secs().max(1)
    }

    pub fn mark_model_capacity_cooldown_for_model(&self, model: &str) {
        let normalized_model = crate::proxy::common::model_mapping::normalize_to_standard_id(model)
            .unwrap_or_else(|| model.to_string());
        self.mark_model_capacity_cooldown(&normalized_model);
    }

    fn mark_model_capacity_cooldown(&self, model: &str) {
        if model.is_empty() {
            return;
        }
        let until = std::time::Instant::now()
            + std::time::Duration::from_secs(Self::MODEL_CAPACITY_COOLDOWN_SECS);
        self.model_capacity_cooldowns
            .insert(model.to_string(), until);
        tracing::warn!(
            "Model capacity cooldown set for {} ({}s)",
            model,
            Self::MODEL_CAPACITY_COOLDOWN_SECS
        );
    }

    fn passes_local_usage_precheck(&self, token: &ProxyToken, model: &str) -> bool {
        if let Some(rpm_limit) = Self::local_rpm_limit(token, model) {
            let current_rpm = self.current_local_rpm(&token.account_id);
            if current_rpm >= rpm_limit {
                tracing::debug!(
                    "Local RPM precheck skipped account {} for model {}: {}/{} requests in {}s",
                    token.email,
                    model,
                    current_rpm,
                    rpm_limit,
                    Self::LOCAL_RPM_WINDOW_SECS
                );
                return false;
            }
        }

        if let Some(rpd_limit) = Self::local_rpd_limit(token, model) {
            let since = (chrono::Local::now() - chrono::Duration::days(1)).timestamp();
            match crate::modules::token_stats::count_account_model_family_requests_since(
                &token.email,
                model,
                since,
            ) {
                Ok(count) if count >= rpd_limit => {
                    tracing::debug!(
                        "Local RPD precheck skipped account {} for model family {}: {}/{} successful requests in 24h",
                        token.email,
                        model,
                        count,
                        rpd_limit
                ...
                Err(e) => {
                    tracing::debug!(
                        "Local RPD precheck unavailable for account {}: {}",
                        token.email,
                        e
                    );
                }
    ...
    fn acquire_account_load_slot(&self, account_id: &str) -> AccountLoadGuard {
        let counter = self
            .account_inflight
            .entry(account_id.to_string())
        ...
        tracing::debug!(
            "Account load acquired: account_id={}, inflight={}",
            account_id,
            current
        );
        AccountLoadGuard {
    ...
    /// 从前 5 个候选中随机选 2 个，优先选择当前 in-flight 更低的账号，其次选择配额更高的账号。
    /// 返回选中的索引
    ///
    /// # 参数
    ...
    fn select_with_p2c<'a>(
        ...
        // 如果池内存在低于软并发阈值的账号，先排除已经忙碌的账号，避免并发请求打爆同一账号。
        let pool_size = available.len().min(Self::P2C_POOL_SIZE);
        let mut pool: Vec<&ProxyToken> = available.iter().take(pool_size).copied().collect();
        if pool.len() > 1 {
            let under_soft_limit: Vec<&ProxyToken> = pool
                .iter()
                .copied()
                .filter(|t| {
                    self.account_inflight_count(&t.account_id) < Self::ACCOUNT_SOFT_INFLIGHT_LIMIT
                })
                .collect();
            if !under_soft_limit.is_empty() {
                pool = under_soft_limit;
            }
        }

        let mut rng = rand::thread_rng();

        let pick1 = rng.gen_range(0..pool.len());
        let pick2 = rng.gen_range(0..pool.len());
        // 确保选择不同的两个候选
        ...
        let load1 = self.account_inflight_count(&c1.account_id);
        let load2 = self.account_inflight_count(&c2.account_id);

        // 当前负载优先，其次选择配额更高的
        let selected = if load1 < load2 {
            c1
        } else if load2 < load1 {
            c2
        } else if c1.remaining_quota.unwrap_or(0) >= c2.remaining_quota.unwrap_or(0) {
            c1
        } else {
            c2
        };
        ...
        tracing::debug!(
            "🎲 [P2C] Selected {} (quota={}%, load={}) from [{}(quota={}%, load={}), {}(quota={}%, load={})]",
            selected.email, selected.remaining_quota.unwrap_or(0), self.account_inflight_count(&selected.account_id),
            c1.email, c1.remaining_quota.unwrap_or(0), load1,
            c2.email, c2.remaining_quota.unwrap_or(0), load2
        );
    ...
    pub async fn get_token(
        ...
        target_model: &str,
    ) -> Result<(String, String, String, String, u64, AccountLoadGuard), String> {
    ...
    async fn get_token_internal(
        ...
        let capacity_wait = self.model_capacity_remaining_wait(&normalized_target);
        if capacity_wait > 0 {
            return Err(format!(
                "Model capacity cooldown active for {}. Wait {}s.",
                normalized_target, capacity_wait
            ));
        }
                ...
                .cloned()
            {
                ...
                match Self::get_account_state_on_disk(&preferred_token.account_path).await {
                    ...
                    OnDiskAccountState::Enabled => {
                        ...
                        let local_usage_ok =
                            self.passes_local_usage_precheck(&preferred_token, &normalized_target);

                        if !is_rate_limited && !is_quota_protected && local_usage_ok {
                            tracing::info!(
                            ...
                            } else if !local_usage_ok {
                                tracing::warn!("🔒 [FIX #820] Preferred account {} is locally throttled for {}, falling back to round-robin", preferred_token.email, target_model);
                            } else {
        ...
        for attempt in 0..total {
                ...
                && scheduling.mode != SchedulingMode::PerformanceFirst
            {
                ...
                if let Some(bound_id) = self.session_accounts.get(sid).map(|v| v.clone()) {
                        ...
                        tokens_snapshot.iter().find(|t| t.account_id == bound_id)
                    {
                        ...
                        {
                            let sticky_load = self.account_inflight_count(&bound_id);
                            let locally_available =
                                self.passes_local_usage_precheck(bound_token, &normalized_target);
                            if !locally_available {
                                tracing::debug!(
                                    "Sticky Session: Bound account {} is locally throttled for {}, selecting another account.",
                                    bound_token.email,
                                    normalized_target
                                );
                            } else if scheduling.mode != SchedulingMode::CacheFirst
                                && sticky_load >= Self::ACCOUNT_SOFT_INFLIGHT_LIMIT
                                && total > 1
                            {
                                tracing::debug!(
                                    "Sticky Session: Bound account {} is busy (inflight={}), selecting a lower-load account.",
                                    bound_token.email,
                                    sticky_load
                                );
                            } else {
                ...
                if let Some((account_id, last_time)) = &last_used_account_id {
                    ...
                    if last_time.elapsed().as_secs() < 60 && !attempted.contains(account_id) {
                            ...
                            tokens_snapshot.iter().find(|t| &t.account_id == account_id)
                        {
                            ...
                            {
                                let found_load = self.account_inflight_count(&found.account_id);
                                let locally_available =
                                    self.passes_local_usage_precheck(found, &normalized_target);
                                if !locally_available {
                                    tracing::debug!(
                                        "60s Window: Last account {} is locally throttled for {}, skipping",
                                        found.email,
                                        normalized_target
                                    );
                                } else if scheduling.mode != SchedulingMode::CacheFirst
                                    && found_load >= Self::ACCOUNT_SOFT_INFLIGHT_LIMIT
                                    && total > 1
                                {
                                    tracing::debug!(
                                        "60s Window: Last account {} is busy (inflight={}), skipping",
                                        found.email,
                                        found_load
                                    );
                                } else {
                ...
                if target_token.is_none() {
                    ...
                    for t in &tokens_snapshot {
                        if !self
                            ...
                            .await
                            && self.passes_local_usage_precheck(t, &normalized_target)
                        {
            ...
            let mut token = match target_token {
                ...
                None => {
                    ...
                    // Layer 1: Balance/PerformanceFirst only wait for tiny sync windows.
                    // CacheFirst honors the UI-configured max_wait_seconds before falling back.
                    if let Some(wait_sec) = min_wait {
                        let max_inline_wait = if scheduling.mode == SchedulingMode::CacheFirst {
                            scheduling.max_wait_seconds
                        } else {
                            2
                        };

                        if wait_sec <= max_inline_wait {
                            let wait_ms = (wait_sec as f64 * 1000.0) as u64;
                            tracing::warn!(
                                "All accounts rate-limited but shortest wait is {}s. Waiting {}ms in {:?} mode...",
                                wait_sec, wait_ms, scheduling.mode
                            );
    ...
    pub async fn mark_rate_limited_async(
        ...
        let reason =
            if error_lower.contains("model_capacity") || error_lower.contains("model capacity") {
                crate::proxy::rate_limit::RateLimitReason::ModelCapacityExhausted
            } else if error_lower.contains("exhausted") || error_lower.contains("quota") {
        ...
        if matches!(
            ...
            tracing::debug!(
                "账号 {} 收到 MODEL_CAPACITY_EXHAUSTED，不写账号限流；由重试耗尽路径设置模型 cooldown",
                account_id
            );
            return;
        }
...
mod tests {
    ...
    async fn test_fixed_account_mode_skips_preferred_when_disabled_on_disk_without_reload() {
        ...
        let (_token, _project_id, email, account_id, _wait_ms, _load_guard) = manager
    ...
    async fn test_sticky_session_skips_bound_account_when_disabled_on_disk_without_reload() {
        ...
        let (_token, _project_id, _email, account_id, _wait_ms, _load_guard) = manager
            .get_token("gemini", false, Some("sid1"), "gemini-1.5-flash")
            .await
            .unwrap();
    ...
    #[test]
    fn test_p2c_prefers_lower_inflight_over_quota_hotspot() {
        let manager = TokenManager::new(PathBuf::from("/tmp/test"));

        let low_quota = create_test_token("low@test.com", Some("PRO"), 1.0, None, Some(20));
        let high_quota = create_test_token("high@test.com", Some("PRO"), 1.0, None, Some(80));

        let _guard1 = manager.acquire_account_load_slot("high@test.com");
        let _guard2 = manager.acquire_account_load_slot("high@test.com");
        ...
        let result = manager.select_with_p2c(&candidates, &attempted, "claude-sonnet", false);
        assert!(result.is_some());
        assert_eq!(result.unwrap().email, "low@test.com");
    }

    #[test]
    fn test_account_load_guard_releases_on_drop() {
        let manager = TokenManager::new(PathBuf::from("/tmp/test"));

        {
            let _guard = manager.acquire_account_load_slot("acc1");
            assert_eq!(manager.account_inflight_count("acc1"), 1);
        }

        assert_eq!(manager.account_inflight_count("acc1"), 0);
    }

    #[test]
    fn test_local_rpm_precheck_skips_free_pro_hotspot() {
        let manager = TokenManager::new(PathBuf::from("/tmp/test"));
        let token = create_test_token("free@test.com", Some("FREE"), 1.0, None, Some(80));

        assert!(manager.passes_local_usage_precheck(&token, "claude-sonnet"));
        manager.record_local_dispatch(&token.account_id);
        manager.record_local_dispatch(&token.account_id);

        assert!(
            !manager.passes_local_usage_precheck(&token, "claude-sonnet"),
            "free/pro-model local RPM limit should skip after two dispatches in the current window"
        );
    }

    #[test]
    fn test_model_capacity_cooldown_blocks_model_temporarily() {
        let manager = TokenManager::new(PathBuf::from("/tmp/test"));

        assert_eq!(manager.model_capacity_remaining_wait("claude-sonnet"), 0);
    ...
    #[tokio::test]
    async fn test_model_capacity_mark_rate_limited_does_not_preempt_retry() {
        let manager = TokenManager::new(PathBuf::from("/tmp/test"));
        let bodies = [
        ...
        for body in bodies {
            manager
                .mark_rate_limited_async("acc@test.com", 429, None, body, Some("claude-sonnet"))
                .await;

            assert_eq!(manager.model_capacity_remaining_wait("claude-sonnet"), 0);
            assert!(
                !manager
                    .is_rate_limited("acc@test.com", Some("claude-sonnet"))
                    .await,
                "MODEL_CAPACITY_EXHAUSTED should not lock the account before in-place retries finish"
            );
        }
    }
    ...
    async fn test_load_account_clears_protected_models_when_overage_enabled() {
        ...
        let overage_token = manager
            .tokens
            .get("acc_overage")
            .expect("overage account loaded");
    ...
    async fn test_p2c_selects_overage_account_over_protected_normal_account() {
        ...
        let candidates: Vec<ProxyToken> =
            manager.tokens.iter().map(|e| e.value().clone()).collect();
