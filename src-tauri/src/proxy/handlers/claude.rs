...
pub async fn handle_messages(
    ...
    for attempt in 0..max_attempts {
        ...
        {
            // 判断是否需要轮换账号
            force_rotate_next = should_rotate_account(status_code, Some(&retry_strategy));
            ...
            if matches!(retry_strategy, RetryStrategy::ModelCapacityRetry(_))
                && attempt + 1 >= max_attempts
            {
                token_manager.mark_model_capacity_cooldown_for_model(&request_with_mapped.model);
            }
            continue;
        } else {
...
async fn call_gemini_sync(
    ...
    let (access_token, project_id, _, account_id, _wait_ms, _load_guard) = token_manager
