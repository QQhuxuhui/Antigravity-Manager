...
#[test]
fn test_retry_strategy_model_capacity_retries_in_place() {
    for status in [429, 503] {
        ...
        match strategy {
            RetryStrategy::ModelCapacityRetry(d) => assert_eq!(d, Duration::from_secs(1)),
            other => panic!(
                "Expected ModelCapacityRetry for {}, got {:?}",
                status, other
            ),
        }
        assert!(!should_rotate_account(status, Some(&strategy)));
    }
}
...
fn test_retry_strategy_429_short_delay_grace_retries_until_7s() {
    ...
    assert!(
        matches!(strategy, RetryStrategy::GraceRetry(_)),
        "Expected GraceRetry for short retryDelay, got {:?}",
        strategy
    );
